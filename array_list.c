...
array_list_t* create_array_list(void)
{
    ...
    if (new_list == NULL) {
        ...
        new_list->entries = malloc(INITIAL_CAPACITY * sizeof(void*));
        if (new_list->entries == NULL) {
            //gotta free up the list now, since not enough space for pointers
            free(new_list);
            return NULL;
        } else {
            new_list->size = 0;
...
array_list_t* shallow_copy_array_list(array_list_t *list_to_copy)
{
    ...
    //create a new array
    copy->entries = malloc(copy->capacity * sizeof(void*));
